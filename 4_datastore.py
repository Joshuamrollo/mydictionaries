...
def main():
    file = open("retail_space.csv", mode="w")

    file.write("room-number,use,sq-ft,price\n")
    for item in datastore["medical"]:
        file.write(str(item["room-number"]) + "," + item["use"] + "," + str(item["sq-ft"]) + "," + str(item["price"]) + "\n")
